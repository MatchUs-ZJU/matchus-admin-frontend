    ...
    <PageContainer>
      ...
      >
        <Alert
          ...
          message={
            currentRow?.identified as number === IDENTIFY_STATUS.SUCCESS ? '认证通过'
              : currentRow?.identified as number === IDENTIFY_STATUS.FAIL ? '审核不通过'
                : currentRow?.identified as number === IDENTIFY_STATUS.IDENTIFYING ? '待审核'
                  : '用户未认证'
          }
          type={
            currentRow?.identified as number === IDENTIFY_STATUS.SUCCESS ? 'success'
              : currentRow?.identified as number === IDENTIFY_STATUS.FAIL ? 'error'
        ...
        <Divider/>
        <Space>
          <Button type="primary" size='large' style={{width: '96px'}} onClick={handleIdentifyPass}>通过</Button>
          <Button type="primary" size='large' style={{width: '96px'}} danger
                  onClick={() => setIdentifyDenyReasonModalVisible(true)}>不通过</Button>
        </Space>
      </Drawer>
      <Modal title='请选择失败理由'
      ...
      >
        {
          ...
          <Form
            ...
            onFinish={handleEditRegisterForm}
          >
            <Form.Item
              label="姓名"
              name="realname"
            ...
            >
              <Radio.Group>
                ...
                <Radio value={2}>毕业3年内校友</Radio>
                <Radio value={3}>毕业3年以上校友</Radio>
              </Radio.Group>
            </Form.Item>
            <Form.Item
              label="学院"
